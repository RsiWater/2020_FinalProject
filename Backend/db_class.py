...
class Account:
    item,detail,position,receipt,note,dbfile,query='','','','','','',''
    ...
    def get_money(self):
        return self.money
    def set_number(self, number):        #set id(insert時使用)
        self.number=number
    def get_number(self):
        return self.number
    ...
    def get_detail(self):
        return self.detail
    def set_position(self, position):  #set位置
    ...
    def insert(self,item,year,month,day,money,number,detail,position,receipt,note,status):
        self.con.execute('insert into record(金額,年,月,日,分類,細項,位置,發票,備註,收支屬性,id)values({},{},{},{},"{}","{}","{}","{}","{}",{},{});'.format(money,year,month,day,item,detail,position,receipt,note,status,number))
        self.con.commit()
    def delete(self,key):
        self.con.execute('delete from record where id={};'.format(key))
        self.con.commit()
    def update(self,query,key):
        self.con.execute('update record set {} where id={};'.format(query,key))
        self.con.commit()
    def select(self,key):
        datas=[]
        data=self.con.execute('select * from record where id={};'.format(key))
        for i in data:
            for j in i:
                datas.append(j)
