...
class Account:
    ...
    def insert(self):
        self.con.execute('insert into record(金額,年,月,日,分類,細項,位置,發票,備註,收支屬性,id)values({},{},{},{},"{}","{}","{}","{}","{}",{},{});'.format(self.money,self.year,self.month,self.day,self.item,self.detail,self.position,self.receipt,self.note,self.status,self.number))
        self.con.commit()
    def delete(self):
